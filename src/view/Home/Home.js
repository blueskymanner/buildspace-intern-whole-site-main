...
function Home(props) {
    ...
    return (
        <Flex className={!collapse ? "container mobile-home" : "container mobile-home wider-width"} justifyContent='center' alignItems='center' flexDirection='column' mt={50} >
            {!account && <Flex justifyContent='center' flexDirection='column' alignItems='center' mt={-30}>
                <WalletConnect />
            </Flex>}
            <Flex justifyContent='center' alignItems='center' flexDirection={isMobile ? 'column' : 'row'} mt={isMobile && account && 50}>
                <WaveMe />
                <Flex m={10} />
                <MintNFT />
                <Flex m={10} />
